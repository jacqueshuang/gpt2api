...
## 3. All-in-One GHCR 单镜像部署

GitHub Actions 会构建并发布 all-in-one 镜像到 GHCR。该镜像包含 user/admin 前端、`api` / `admin` / `openai` / `worker` 后端进程、nginx 和 supervisord；MySQL 与 Redis 仍为外部依赖。
...
容器公开端口为 `17080`、`17088`、`17200`。OpenAI 后端进程在容器内只监听 `127.0.0.1:17201`，由 nginx 对外代理到 `17200`。

---

## 4. Docker Compose 部署（单机 / 小规模）
...
### 4.2 `docker-compose.prod.yml` 示例
...
### 8.1 CI（GitHub Actions / GitLab CI）
